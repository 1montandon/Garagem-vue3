...
const defaultModelo = { id: null, nome: "", marca:"", categoria: Object };
...
onMounted(async () => {
  ...
  modelos.value = await modelosApi.buscarTodasAsMarcas();
});
...
async function salvar() {
  if (modelo.nome.length >= 100 || modelo.nacionalidade.length >= 100) {
    alert("mais de 50");
  } else if (modelo.id) {
    await modelosApi.atualizarModelos(modelo);
  } else {
    await modelosApi.adicionarModelos(modelo);
  }
  modelos.value = await modelosApi.buscarTodasOsModelos();
  limpar();
}
